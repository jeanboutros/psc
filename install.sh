...
print_summary() {
    ...
    if [ "$merge_count" -gt 0 ]; then
        ...
        echo ""
        echo -e "  ${YELLOW}Review the merge files in .opencode/merge/ and resolve conflicts.${NC}"
        echo -e "  ${YELLOW}After merging, remove the .merge.md files.${NC}"
    fi
    ...
    echo -e "  1. Review and customise AGENTS.md in your project root"
    echo -e "  2. Adjust the skill loading protocol in agents for your project"
    echo -e "  3. Run: ${BLUE}bash docs/pipeline/scripts/t1-check.sh${NC} to verify T1 checks"
    echo -e "  4. Start using OpenCode with: ${BLUE}opencode${NC}"
    echo ""
}
